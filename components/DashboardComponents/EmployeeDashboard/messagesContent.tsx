							...
							<div className="flex items-start gap-3">
								...
								<div className="flex-1 min-w-0">
									...
									<div className="flex items-center justify-between">
										<p
											className={`text-sm truncate ${!conversation.lastMessage.isRead ? "font-medium text-foreground" : "text-muted-foreground"}`}
										>
			...
			<div className="flex-1 flex flex-col">
				...
				<div className="p-4 border-b border-border bg-background">
					...
					<div className="mt-3 p-3 bg-muted/50 rounded-lg">
						<div className="flex items-center gap-2">
							...
							<span
								className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${MESSAGES_DATA.conversations.find((c) => c.id === selectedConversation.id)?.statusColor || "bg-gray-100 text-gray-800"} ml-auto`}
							>
				...
				<div className="flex-1 overflow-y-auto p-4 space-y-4">
					{selectedConversation.messages.map((message) => (
						...
						>
							<div
								className={`flex gap-2 max-w-[70%] ${message.sender === "user" ? "flex-row-reverse" : "flex-row"}`}
							>
